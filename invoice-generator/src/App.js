...
function App() {
  ...
  return (
   ...
   <main className="p-5 m-5 md:max-w-xl md:mx-auto lg:max-w-2xl  bg-white rounded shadow">
          ...
          <ReactToPrint
            trigger={() => (
              <button className="bg-blue-500 mb-5 text-white font-bold  py-2 px-8 rounded shadow border-2 border-blue-500 
   ...
   { showInvoice ?<><div ref={componentRef} className='p-5'>
     ...
     <Notes notes={notes}/>
     <Footer name={name} address={address} email={email} website={website} phone={phone}
      bankName={bankName} bankAccount={bankAccount}/>
     </div>  <button onClick={()=>setShowInvoice(false)} className=" mt-5 bg-blue-500 text-white font-bold  py-2 px-8 rounded shadow border-2 border-blue-500 
...
></textarea>
     ...
     hover:bg-transparent hover:text-blue-500 transition-all duration-300">Preview Invoice</button></div></>
     )}
   </main>
   </>
  )
